...
def clahe_histo(file_name):
	...
	img = cv2.imread('/home/sumitsaha/Desktop/{}.jpg'.format(file_name),0) 
...
def haar_cascade(file_name):
	...
	img = cv2.imread('/home/sumitsaha/Desktop/{}.jpg'.format(file_name))
	...
	cv2.imshow('{}-haar-cascade'.format(file_name),img)
	cv2.waitKey(0)
	cv2.destroyAllWindows()

def canny_edge(file_name):

	img = cv2.imread('/home/sumitsaha/Desktop/{}.png'.format(file_name),cv2.IMREAD_GRAYSCALE)
 
	filter = cv2.Canny(img,100,200)
 
	cv2.imshow('Original - Canny Edge',img)
	cv2.imshow('Canny Edge',filter)
